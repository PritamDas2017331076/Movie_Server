...
func callFunc() {
	...
	router.HandleFunc("/addmoviegenre/{id}", Auth.IsAuthenticated(AddMovieGenre)).Methods("POST")
	router.HandleFunc("/deletemoviedirector/{id}", Auth.IsAuthenticated(DeleteMovieDirector)).Methods("DELETE")
	router.HandleFunc("/addmoviedirector/{id}", Auth.IsAuthenticated(AddMovieDirector)).Methods("POST")
